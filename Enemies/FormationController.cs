...
public class FormationController : MonoBehaviour
{
    ...
    //void SpawnEnemies()
    //{
    //    // loopt over alle objecten via Transform ipv GameObject, en "Spawnt" deze afhankelijk van de positie 
    //    foreach (Transform child in transform)
    //    {
    //        GameObject enemy = Instantiate(enemyPrefab, child.position, Quaternion.identity) as GameObject;
    //        enemy.transform.parent = child; // transform;// transform of the formation
    //    }
    //}
    ...
    void SpawnUntilFull()
    {
        ...
        if (freePosition && numberOfRounds < 2)
        {
            GameObject enemy = Instantiate(enemyPrefab, freePosition.position, Quaternion.identity) as GameObject;
            enemy.transform.parent = freePosition; // transform;// transform of the formation
        }
        if (NextFreePosition())
        {
